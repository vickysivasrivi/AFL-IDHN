...
SERVER_ID: str = f"AFL_Adaptive_Server_{socket.gethostname()}"
...
def build_model(n_features: int, num_classes: int) -> Model:
    """Creates a lightweight CNN-LSTM model for intrusion detection.

    This architecture is used by both the server and the clients to ensure
    consistency.

    Args:
        n_features: The number of input features for the model.
        num_classes: The number of output classes (e.g., attack types).

    Returns:
        A compiled TensorFlow Keras model.
    """
    model_input = Input(shape=(1, n_features))
    # A Conv1D layer is effective for finding patterns in sequence-like data.
    x = Conv1D(filters=64, kernel_size=1, activation="relu")(model_input)
    # The LSTM layer captures temporal or sequential dependencies in the features.
...
class AggregationServer:
    """Orchestrates the adaptive federated learning process using MQTT."""
    ...
    def __init__(self) -> None:
        ...
        print("[SERVER - ADAPTIVE] Initial global model created.")
    ...
    def _on_connect(self, client, userdata, flags, rc, properties=None) -> None:
        ...
        if rc == 0:
            print(f"[SERVER - ADAPTIVE] Connected to MQTT Broker")
            self.is_connected = True
            client.subscribe(TOPIC_CLIENT_READY)
            client.subscribe(TOPIC_CLIENT_UPDATE)
            print(f"[SERVER - ADAPTIVE] Subscribed to client topics.")
        else:
            print(f"[SERVER - ADAPTIVE] Failed to connect, return code {rc}")
            self.is_connected = False

    def _on_disconnect(self, client, userdata, flags,rc, properties=None) -> None:
        """Callback for MQTT disconnection."""
        print(f"[SERVER - ADAPTIVE] Disconnected from broker.")
        self.is_connected = False
    ...
    def _on_message(self, client, userdata, msg) -> None:
        ...
        try:
            ...
            print(f"[SERVER] Received non-UTF8 payload on topic {topic}")
            return
        ...
        if topic == TOPIC_CLIENT_READY:
            ...
            if client_id:
                ...
                print(
                    f"[SERVER - ADAPTIVE] Client ready: {client_id} ({len(self.ready_clients)} total)"
                )
            ...
            try:
                ...
                if data.get("round") == self.current_round:
                    ...
                    print(
                        f"[SERVER - ADAPTIVE] Received update from {data['client_id']}"
                    )
                else:
                    print(
                        f"[SERVER - ADAPTIVE] Received stale update from {data['client_id']}. Ignoring."
                    )
            except Exception as e:
                print(f"[SERVER - ADAPTIVE] Error processing update: {e}")

    def start_new_round(self) -> None:
        """Broadcasts the start signal and current global model weights layer-by-layer."""
        print(
            f"[SERVER - ADAPTIVE] Broadcasting weights for round {self.current_round}"
        )
        weights = self.global_model.get_weights()

        header = {"round": self.current_round, "num_layers": len(weights)}
        self.mqttc.publish(
            TOPIC_GLOBAL_WEIGHTS_HEADER, json.dumps(header), qos=1, retain=False
        )

        # Publish each layer's weights to a unique sub-topic to avoid broker limits
        for i, layer_weights in enumerate(weights):
            payload = serialize_weights([layer_weights])
            self.mqttc.publish(
                f"{TOPIC_GLOBAL_WEIGHTS_CHUNK}/{i}", payload, qos=1, retain=False
            )
            time.sleep(0.05)  # Small delay to prevent rate-limiting on public brokers
        ...
        print(f"[SERVER - ADAPTIVE] Published start signal for round {self.current_round}.")

    def aggregate_updates(self) -> None:
        """Aggregates client updates using the adaptive method."""
        if not self.updates:
            print("[SERVER - ADAPTIVE] No updates to aggregate.")
            return

        new_weights = adaptive_federated_averaging(self.updates)
        if new_weights:
            self.global_model.set_weights(new_weights)
            print("[SERVER - ADAPTIVE] Global model updated.")

    def run_server(self) -> None:
        """Main server loop to manage the federated learning process."""
        self.mqttc.loop_start()
        print("[SERVER - ADAPTIVE] Waiting for MQTT connection...")
        while not self.is_connected:
            time.sleep(1)

        print(
            f"[SERVER - ADAPTIVE] Waiting for {MIN_CLIENTS_PER_ROUND} client(s) to connect..."
        )
        ...
        for r in range(1, TOTAL_ROUNDS + 1):
            ...
            print(f"[SERVER - ADAPTIVE] Waiting for client updates ({ROUND_TIMEOUT}s)...")
            time.sleep(ROUND_TIMEOUT)

            if len(self.updates) >= MIN_CLIENTS_PER_ROUND:
                self.aggregate_updates()
            else:
                print(f"[SERVER - ADAPTIVE] Round timed out. Insufficient updates.")

            time.sleep(5)

        # Save the final model weights after all rounds are complete
        print("\n[SERVER - ADAPTIVE] All rounds completed. Saving final global model...")
        save_path = os.path.join("saved_models", "final_global_model_adaptive.weights.h5")
        self.global_model.save_weights(save_path)
        print(f"[SERVER - ADAPTIVE] Model saved to '{save_path}'. Shutting down.")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n[SERVER - ADAPTIVE] Manual shutdown.")