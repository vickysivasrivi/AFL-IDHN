...
#     adaptive_federated_averaging, quantize_model_tflite
# )
# # --- Configuration ---
...
#     def start_new_round(self):
#         print(f"[SERVER] Broadcasting start signal and QUANTIZED weights for round {self.current_round}")

#         quantized_model_bytes = quantize_model_tflite(self.global_model)
#         print(f"[SERVER] Model quantized. Original size approx: {self.global_model.count_params() * 4 / 1024:.2f} KB, Quantized size: {len(quantized_model_bytes) / 1024:.2f} KB")

#         # --- DEFINITIVE FIX: Use the correct topic variable ---
#         # The single quantized model should be sent to the topic that the client expects for the main payload.
#         # Let's define a specific topic for this to be clear.
#         TOPIC_QUANTIZED_MODEL = "afl/global/quantized_model"
        
#         self.mqttc.publish(TOPIC_QUANTIZED_MODEL, payload=quantized_model_bytes, qos=1, retain=False)
#         print(f"[SERVER] Published quantized global model to '{TOPIC_QUANTIZED_MODEL}'.")
...
# server_federated_mqtt.py (Final Version for Training)
...
from utils_mqtt_fl import (
    serialize_weights, deserialize_weights, serialize_message, deserialize_message,
    adaptive_federated_averaging # Use the adaptive function
)
...
class AggregationServer:
    ...
    def on_connect(self, client, userdata, flags, rc, properties=None):
        ...
        if rc == 0:
            ...
            print(f"[SERVER] Failed to connect, return code {rc}"); self.is_connected = False
            
    def on_disconnect(self, client, userdata, flags, rc, properties=None):
        # (Unchanged)
        print(f"[SERVER] Disconnected with rc={rc}. The loop will pause."); self.is_connected = False

    def on_message(self, client, userdata, msg):
        # (This is the working version from before)
        topic = msg.topic;
        try: payload_str = msg.payload.decode("utf-8")
        except UnicodeDecodeError: payload_str = None
        if topic == TOPIC_CLIENT_READY and payload_str:
            client_id = deserialize_message(payload_str).get("client_id")
            if client_id: self.ready_clients.add(client_id); print(f"[SERVER] Client ready: {client_id} ({len(self.ready_clients)} total)")
        elif topic == TOPIC_CLIENT_UPDATE and payload_str:
            try:
                data = deserialize_message(payload_str)
                if data.get("round") == self.current_round:
                    ...
                    print(f"[SERVER] Received update from {data['client_id']} for round {self.current_round} ({len(self.updates)}/{MIN_CLIENTS_PER_ROUND})")
                else: print(f"[SERVER] Received stale update from {data['client_id']} for round {data.get('round')}. Ignoring.")
            except Exception as e: print(f"[SERVER] Error processing update: {e}")

    def start_new_round(self):
        # --- REVERTED to sending Keras weights layer-by-layer ---
        print(f"[SERVER] Broadcasting start signal and full weights for round {self.current_round}")
        weights = self.global_model.get_weights()
    ...
    def aggregate_and_broadcast(self):
        # (Unchanged - uses the adaptive function correctly)
        new_weights = adaptive_federated_averaging(self.updates)
        if new_weights: self.global_model.set_weights(new_weights); print("[SERVER] Global model updated successfully using adaptive weights.")
    ...
    def run_server(self):
        ...
        print("[SERVER] All rounds completed. Saving final global model...")
        save_path = os.path.join("saved_models", "final_global_model_adaptive.weights.h5")
        self.global_model.save_weights(save_path)
        print("[SERVER] Model saved to 'final_global_model_adaptive.weights.h5'. Shutting down.")
        self.mqttc.loop_stop()
