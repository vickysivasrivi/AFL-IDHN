...
# --- IMPORTANT: We are now importing the original, non-adaptive function ---
from utils_mqtt_fl import (
    serialize_weights, deserialize_weights, serialize_message, deserialize_message,
    weighted_average_adaptive # This is the baseline function
)
...
class AggregationServer:
    def __init__(self):
        ...
        self.updates = []; self.ready_clients = set(); self.current_round = 0; self.is_connected = False
        ...
        self.mqttc.on_connect = self.on_connect; self.mqttc.on_disconnect = self.on_disconnect; self.mqttc.on_message = self.on_message
        self.mqttc.connect(BROKER_HOST, BROKER_PORT, keepalive=60)

    def on_connect(self, client, userdata, flags, rc, properties=None):
        # (Unchanged)
        if rc == 0:
            print(f"[SERVER - BASELINE] Connected with rc={rc}")
            self.is_connected = True
            client.subscribe(TOPIC_CLIENT_READY); client.subscribe(TOPIC_CLIENT_UPDATE)
            print(f"[SERVER - BASELINE] Subscribed to '{TOPIC_CLIENT_READY}' and '{TOPIC_CLIENT_UPDATE}'")
        else:
            print(f"[SERVER - BASELINE] Failed to connect, return code {rc}"); self.is_connected = False
            
    def on_disconnect(self, client, userdata, flags, rc, properties=None):
        # (Unchanged)
        print(f"[SERVER - BASELINE] Disconnected with rc={rc}. The loop will pause."); self.is_connected = False

    def on_message(self, client, userdata, msg):
        # (This is the working version from before, which still collects loss,
        # but the aggregation function below will ignore it)
        topic = msg.topic;
        try: payload_str = msg.payload.decode("utf-8")
        except UnicodeDecodeError: payload_str = None
        if topic == TOPIC_CLIENT_READY and payload_str:
            ...
            try:
                ...
                if data.get("round") == self.current_round:
                    ...
                    self.updates.append({ "weights": weights, "num_samples": data.get("num_samples", 0), "loss": data.get("loss", 1.0), "meta": data.get("meta", {})})
                    print(f"[SERVER - BASELINE] Received update from {data['client_id']} for round {self.current_round} ({len(self.updates)}/{MIN_CLIENTS_PER_ROUND})")
                else: print(f"[SERVER - BASELINE] Received stale update from {data['client_id']} for round {data.get('round')}. Ignoring.")
            except Exception as e: print(f"[SERVER - BASELINE] Error processing update: {e}")

    def start_new_round(self):
        # (Unchanged)
        print(f"[SERVER - BASELINE] Broadcasting start signal and full weights for round {self.current_round}")
        ...
        print(f"[SERVER - BASELINE] Published weights header: {header}")
        time.sleep(0.05)
        for i, layer_weights in enumerate(weights):
            ...
            time.sleep(0.05)
        print(f"[SERVER - BASELINE] Published {len(weights)} weight layers.")
        ...
        print(f"[SERVER - BASELINE] Published round start signal.")

    def aggregate_and_broadcast(self):
        # --- THIS IS THE KEY CHANGE FOR THE BASELINE ---
        ...
        if new_weights:
            ...
            print("[SERVER - BASELINE] Global model updated successfully using simple weights.")
    ...
    def run_server(self):
        ...
        while len(self.ready_clients) < MIN_CLIENTS_PER_ROUND:
            time.sleep(1)
        if not self.is_connected: print("[SERVER - BASELINE] Shutting down, lost connection..."); self.mqttc.loop_stop(); return
        print(f"[SERVER - BASELINE] {len(self.ready_clients)} client(s) ready. Starting rounds.")
        
        while self.current_round < TOTAL_ROUNDS:
            self.current_round += 1
            if not self.is_connected: print("[SERVER - BASELINE] Pausing due to MQTT disconnection..."); time.sleep(5); continue
            print(f"\n[SERVER - BASELINE] = Round {self.current_round} start =")
            self.updates.clear()
            self.start_new_round()
            start_time = time.time()
            while len(self.updates) < MIN_CLIENTS_PER_ROUND and (time.time() - start_time) < ROUND_TIMEOUT: time.sleep(1)
            if len(self.updates) >= MIN_CLIENTS_PER_ROUND:
                print(f"[SERVER - BASELINE] Aggregating {len(self.updates)} updates for round {self.current_round}")
                self.aggregate_and_broadcast()
            else: print(f"[SERVER - BASELINE] Round {self.current_round} timed out. Insufficient updates ({len(self.updates)}).")
            time.sleep(5)
            
        # --- NEW: Save the final model weights for evaluation ---
        print("[SERVER - BASELINE] All rounds completed. Saving final global model...")
        save_path = os.path.join("saved_models", "final_global_model_baseline.weights.h5")
        self.global_model.save_weights(save_path)
        print("[SERVER - BASELINE] Model saved to 'final_global_model_baseline.weights.h5'. Shutting down.")
